...
module.exports = async function handler(req, res) {
    try {
        ...
        if (!process.env.STRIPE_SECRET_KEY) {
            ...
            return res
                .status(500)
                .json({ error: "Stripe not configured - missing secret key" });
        }
        ...
        if (!CONNECT_ACCOUNT_ID) {
            ...
            return res
                .status(500)
                .json({ error: "Stripe Connect account not configured" });
        }
        ...
        const sessionConfig = {
            ...
            success_url: `${
                req.headers?.origin ||
                "https://tribe-fundraiser-final.vercel.app"
            }/?success=true&session_id={CHECKOUT_SESSION_ID}&amount=${amount}&type=${donationType}`,
            cancel_url: `${
                req.headers?.origin ||
                "https://tribe-fundraiser-final.vercel.app"
            }/?canceled=true`,
        ...
        if (priceId) {
            sessionConfig.mode =
                donationType === "monthly" ? "subscription" : "payment";
        ...
        // Use product mapping if productId matches predefined products
            ...
            if (
                mapping.priceId &&
                (donationType === "monthly" || productId !== "one-time")
            ) {
        ...
        // For one-time donations without specific product, use the variable price product
            ...
            if (!amount || amount <= 0) {
                return res
                    .status(400)
                    .json({ error: "Invalid amount for one-time donation" });
            }
        ...
        // Handle specific amount-based product selection
            ...
            return res
                .status(400)
                .json({ error: "Invalid product or amount specified" });
        }
