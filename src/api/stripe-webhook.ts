...
import { insertDonation, updateDonation, getDonationBySessionId } from '../lib/supabase';
...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session) {
  try {
    ...
    const donationRecord = {
      ...
      donation_type: isSubscription ? 'monthly' as const : 'one-time' as const,
      stripe_session_id: session.id,
      stripe_customer_id: typeof session.customer === 'string' ? session.customer : undefined,
      payment_status: session.payment_status === 'paid' ? 'succeeded' as const : 'pending' as const,
