        ...
        setTimeout(() => {
          cardsRef.current.forEach((card, index) => {
            if (card) {
              card.style.transform = 'translateY(0)';
              card.style.opacity = '1';
              card.style.transition = `transform 0.4s ease-out ${index * 0.08}s, opacity 0.4s ease-out ${index * 0.08}s`;
            }
          });
        }, 100);
        ...
        <div className="card-nav-content" aria-hidden={!isExpanded}>
          {items.slice(0, 3).map((item, idx) => (
            <div
              ...
              ref={setCardRef(idx)}
              style={{ 
                backgroundColor: item.label === 'About' ? '#2e7cff' : item.label === 'Pledge' ? '#20fde6' : item.label === 'Explore' ? '#8614ff' : item.bgColor, 
                color: item.label === 'About' ? '#ffffff' : item.label === 'Pledge' ? '#000000' : item.label === 'Explore' ? '#ffffff' : item.textColor 
              }}
            >
              {/* Card Icon - Top Right */}
              <div className="absolute top-4 right-4 opacity-40">
                ...
                {item.label === 'Explore' && (
                  <Rocket className="w-6 h-6" style={{ color: item.textColor }} />
                )}
                {item.label === 'About' && (
