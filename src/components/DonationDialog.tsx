...
export function DonationDialog({ onConfetti, children, darkMode = false }: DonationDialogProps) {
        ...
        <DialogStackContent className={`${
          ...
          {donationAmount === '500' && (
            <div 
              ...
              style={{
                background: 'linear-gradient(135deg, rgba(134, 20, 255, 0.03) 0%, rgba(154, 82, 235, 0.03) 20%, rgba(255, 107, 122, 0.03) 40%, rgba(245, 230, 184, 0.03) 50%, rgba(34, 197, 94, 0.03) 70%, rgba(112, 226, 216, 0.03) 85%, rgba(134, 20, 255, 0.03) 100%)',
          ...
          {donationAmount === '500' && (
            <div 
              ...
              style={{
                background: 'linear-gradient(45deg, rgba(134, 20, 255, 0.4), rgba(154, 82, 235, 0.4), rgba(255, 107, 122, 0.4), rgba(245, 230, 184, 0.4), rgba(34, 197, 94, 0.4), rgba(112, 226, 216, 0.4), rgba(134, 20, 255, 0.4))',
                backgroundSize: '300% 300%',
                filter: 'blur(8px)',
                animation: 'pulse-glow-shifting 3s ease-in-out infinite'
              }}
            />
          )}
          ...
          >
            <X className={`h-4 w-4 ${darkMode ? 'text-slate-300' : 'text-[#4c1d95]'}`} />
          ...
          <DialogStackHeader className="space-y-4 pb-6 relative z-10">
            ...
            <div className="flex justify-center">
              <div className={`relative transition-all duration-500 ${
                donationAmount === '500' ? 'transform scale-110' : ''
              }`}>
                ...
                {donationAmount === '500' && (
                  <div 
                    ...
                    style={{
                      background: 'linear-gradient(90deg, #4C1D95 0%, #4C1D95 33.33%, #ff0138 40%, #ff6b7a 45%, #474dd6 50%, #6de4db 55%, #8614ff 60%, transparent 66.67%, transparent)',
                      backgroundSize: '300% 100%',
                      backgroundPosition: '100% 0',
                      animation: 'chroma-sweep-culture 0.9s ease-in-out 0.1s forwards',
                      filter: 'blur(1px)',
                ...
                <Gift className={`relative z-10 w-12 h-12 transition-all duration-500 ${
                  donationAmount === '500' 
                    ? 'opacity-0' 
                    : 'text-[#8614ff]'
                }`} />
            ...
            <DialogStackTitle className={`text-center font-['Nunito:Bold',_sans-serif] font-bold text-[24px] leading-tight transition-all duration-500 ${
              donationAmount === '500'
            ...
            <DialogStackDescription className={`text-center ${darkMode ? 'text-slate-300' : 'text-[#6b7280]'} font-['Nunito:Regular',_sans-serif] text-[16px] leading-relaxed`}>
              Help us create a place where kids feel seen, celebrated, and empowered to dream big through culturally relevant STEM education.
            </DialogStackDescription>
          </DialogStackHeader>
          ...
          <div className="mb-6 relative z-10">
            <label className={`block ${darkMode ? 'text-slate-200' : 'text-[#4c1d95]'} font-['Nunito:Bold',_sans-serif] font-bold text-[14px] mb-3`}>
              Donation Amount:
            </label>
            <div className="relative group">
              ...
              <input
                ...
                step="25"
                className={`relative z-10 w-full h-[56px] pl-8 pr-3 ${darkMode ? 'bg-slate-800 border-slate-600 text-slate-100' : 'bg-white border-[#e5e7eb] text-[#374151]'} border-2 rounded-[12px] text-[18px] font-['Nunito:Medium',_sans-serif] font-medium focus:border-[#3e1c85] focus:outline-none transition-all duration-300 shadow-sm hover:shadow-md focus:shadow-lg [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none`}
                placeholder="100"
              />
            </div>
          </div>
          ...
          <div className="mb-6 relative z-10">
            <div className="flex justify-between w-full gap-2">
              {['50', '100', '250', '500'].map((amount) => (
                <div key={amount} className="flex-1 relative group">
                  {/* Chroma Glow Effect - Only when selected */}
                  {(donationAmount === amount || (amount === '100' && !['50', '100', '250', '500'].includes(donationAmount))) && (
                    <div 
                  ...
                  <button
                    ...
                    style={(donationAmount === amount || (amount === '100' && !['50', '100', '250', '500'].includes(donationAmount))) ? {
                      boxShadow: '0 8px 32px rgba(0, 0, 0, 0.1), 0 4px 16px rgba(0, 0, 0, 0.05), inset 0 1px 0 rgba(255, 255, 255, 0.9)'
                    } : {}}
                  >
                    <span className={amount === '500' && donationAmount !== amount ? 'gold-shimmer-text' : ''}>
                      ${amount}
                    </span>
                  </button>
                </div>
              ...
              onClick={(e) => {
                ...
                if (donationAmount !== '500') {
                  ...
                  for (let i = 0; i < 50; i++) {
                    ...
                    confetti.style.left = buttonCenter.x + 'px';
                    confetti.style.top = buttonCenter.y + 'px';
                    confetti.style.width = '8px';
                    confetti.style.height = '8px';
                    confetti.style.backgroundColor = ['#8614ff', '#a855f7', '#c084f5', '#FFD700', '#22c55e'][Math.floor(Math.random() * 5)];
                    ...
                    setTimeout(() => {
                      ...
                      const y = Math.sin(angle) * distance - Math.random() * 100;
                      
                      confetti.style.transform = `translate(${x}px, ${y}px) scale(1) rotate(${Math.random() * 360}deg)`;
