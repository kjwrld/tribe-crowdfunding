    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (mostVisible && mostVisible.intersectionRatio > 0.1) {
          console.log('IntersectionObserver setting active section to:', mostVisible.target.id, 'ratio:', mostVisible.intersectionRatio);
              ...
              >
                <span 
                  className={`font-['Nunito:Medium',_sans-serif] font-medium text-[20px] whitespace-nowrap transition-all duration-300 ${
                    isActive ? 'text-white drop-shadow-sm' : 'text-white/85'
                  }`}
                >
                  {section.label}
                </span>
