...
function PricingTier({ tier, title, price, icon, features, isPopular = false, delay = 0, isInView, onConfetti }: PricingTierProps) {
  ...
  return (
    <motion.div
      className={`relative bg-white/95 backdrop-blur-sm rounded-[24px] p-8 border-2 ${
        hasCleanBorderEffect ? 'border-purple-200' : 
        hasBackgroundGlow ? 'border-purple-200 shadow-lg hover:shadow-xl hover:shadow-purple-500/10' : 
        colors.border
    ...
    >
      ...
      {hasBackgroundGlow && (
        <div 
          ...
          style={{
            background: 'linear-gradient(45deg, rgba(134, 20, 255, 0.12), rgba(154, 82, 235, 0.12), rgba(255, 107, 122, 0.12), rgba(245, 230, 184, 0.12), rgba(34, 197, 94, 0.12), rgba(134, 20, 255, 0.12))',
      ...
      <div className="flex flex-col items-center text-center mb-8">
        ...
        >
          {tier === 'ygber' ? (
            <div 
              className="w-8 h-8 crown-container"
            >
        ...
        <div className="mb-2">
          <span className={`font-['Nunito:Bold',_sans-serif] font-bold text-[#8614ff] text-[36px] ${
      ...
      <div className="space-y-4">
        {features.map((feature, index) => (
          ...
          >
            ...
            <span className={`font-['Nunito:Regular',_sans-serif] font-normal text-[#374151] text-[14px] leading-relaxed ${
              tier === 'ygber' && feature.includes('120 students') ? 'shimmer-text' : ''
            }`}>
              {feature}
            </span>
          </motion.div>
      ...
      <motion.button
        ...
        className={`relative group w-full mt-8 bg-gradient-to-r ${
          tier === 'explorer' ? 'from-[#8614ff] to-[#a047ff]' :
          tier === 'steamer' ? 'from-[#6d00e0] to-[#8614ff]' :
  ...
  return (
    <div className="text-center mb-4 lg:mb-16">
      <h2 className="font-['Nunito:Bold',_sans-serif] text-[#d5adff] text-[48px] md:text-[56px] lg:text-[64px] font-bold leading-[1.1] mb-4">
        <span>Help Us </span>
        <span className={`chroma-text ${animateGift ? 'chroma-text-animate' : ''}`}>Reimagine</span>
...
export function ImpactSection({ onConfetti }: { onConfetti?: (position: { x: number; y: number }) => void }) {
      ...
      <div className="flex flex-col items-center relative w-full">
        <div className="box-border content-stretch flex flex-col gap-[56px] items-center px-[32px] relative w-full max-w-7xl">
          ...
          <motion.div
            ...
            transition={{ duration: 0.8, delay: sectionInView ? 1.2 : 0, ease: "easeOut" }}
          >
            <div className="font-['Nunito:Regular',_sans-serif] text-[#8c92ab] text-[16px] leading-relaxed mb-6 lg:mt-12">
              <p className="mb-2">Every contribution directly supports underrepresented students in STEM education.</p>
              <p className="font-['Nunito:Bold',_sans-serif] font-bold text-[#8614ff]">Your gift creates lasting change.</p>
            </div>
